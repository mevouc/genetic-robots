/*************************************************************************
 *  A method for drawing a rotated filledEllipse added by Steve Kroon 20 Jan 2016.
 *  Based on code at http://stackoverflow.com/questions/10872311/how-to-have-rotated-ellipse-shape-in-java
 *
 *  Compilation:  javac SteveDraw.java
 *  Execution:    java SteveDraw
 *
 ...
 *  @author Kevin Wayne
 */
public final class SteveDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static SteveDraw std = new SteveDraw();
    ...
    *************************************************************************/
    ...
    /**
     * Draw a filled ellipse with given semimajor and semiminor axes, centered on (x, y), and rotated theta radians.
     * Added by Steve Kroon.
     * @param x the x-coordinate of the center of the ellipse
     * @param y the y-coordinate of the center of the ellipse
     * @param semiMajorAxis is the semimajor axis of the ellipse
     * @param semiMinorAxis is the semiminor axis of the ellipse
     * @throws IllegalArgumentException if either of the axes are negative
     */
    public static void filledEllipse(double x, double y, double semiMajorAxis, double semiMinorAxis, double theta) {
        if (semiMajorAxis < 0) throw new IllegalArgumentException("ellipse semimajor axis must be nonnegative");
        if (semiMinorAxis < 0) throw new IllegalArgumentException("ellipse semiminor axis must be nonnegative");
        double xs = scaleX(x);
        double ys = scaleY(y);
        ...
        if (ws <= 1 && hs <= 1) pixel(x, y);
            ...
            Shape s = AffineTransform.getRotateInstance(theta, xs, ys).createTransformedShape(new Ellipse2D.Double(xs - ws/2, ys - hs/2, ws, hs));
            offscreen.fill(s);
        }
        draw();
    }
    ...
    /**
     * Draw an equilateral triangle centered on (x, y) with the given radius and
     * the given angle.
     ...
     */
    public static void triangle(double x, double y, double r, double theta)
    {
      if (r < 0)
        throw new IllegalArgumentException("Radius cannot be negative.");
      double[] xs = { Math.cos(Math.toRadians(theta)) * r,
                      Math.cos(Math.toRadians(theta - 120)) * r,
                      Math.cos(Math.toRadians(theta + 120)) * r };
      double[] ys = { Math.sin(Math.toRadians(theta)) * r,
                      Math.sin(Math.toRadians(theta - 120)) * r,
                      Math.sin(Math.toRadians(theta + 120)) * r };
      for (int i = 0; i < 3; i++)
        xs[i] += x;
      for (int i = 0; i < 3; i++)
    ...
    /**
     * Draw an equilateral filled triangle centered on (x, y) with the given
     * radius and the given angle.
     * @param x x-coordinate of the triangle
     * @param y y-coordinate of the triangle
     * @param r radius of the triangle
     * @param theta angle of the triangle
     */
    public static void filledTriangle(double x, double y, double r, double theta)
    {
      if (r < 0)
        throw new IllegalArgumentException("Radius cannot be negative.");
      double[] xs = { Math.cos(Math.toRadians(theta)) * r,
    ...
    *************************************************************************/
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = SteveDraw.class.getResource(filename);
    ...
    /**
     * Draw picture (gif, jpg, or png) centered on (x, y), rescaled to a new
     * size equals to n times the original one.
     * @param x the center x coordinate of the image
     * @param y the center y coordinate of the image
     * @param s the name of the image/picture, e.g., "ball.gif"
     * @param n the factor to apply on the image
     * @throws IllegalArgumentException if the width height are negative
     * @throws IllegalArgumentException if the image is corrupt
     */
/*    public static void picture(double x, double y, String s, double n)
    {
...
*/
     ...
     * given number of degrees, rescaled to a new size equals to n times the
     * original one.
     * @param x the center x-coordinate of the image
     * @param y the center y-coordinate of the image
     * @param s the name of the image/picture, e.g., "ball.gif"
    ...
    public static void text(double x, double y, String s) {
        ...
        if (s.contains("\n"))
        {
          ...
          for(String line : s.split("\n"))
            text(x, y -= (userY(metrics.getDescent()) * 0.075), line);
          return;
        }
        double xs = scaleX(x);
        double ys = scaleY(y);
    ...
    *************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(SteveDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            SteveDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    *************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = SteveDraw.userX(e.getX());
            mouseY = SteveDraw.userY(e.getY());
            mousePressed = true;
        }
    }
    ...
    *************************************************************************/
    ...
    public static void main(String[] args) {
        SteveDraw.square(.2, .8, .1);
        SteveDraw.filledSquare(.8, .8, .2);
        SteveDraw.circle(.8, .2, .2);

        SteveDraw.setPenColor(SteveDraw.BOOK_RED);
        SteveDraw.setPenRadius(.02);
        SteveDraw.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        SteveDraw.setPenRadius();
        SteveDraw.setPenColor(SteveDraw.BOOK_BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        SteveDraw.filledPolygon(x, y);

        // text
        SteveDraw.setPenColor(SteveDraw.BLACK);
        SteveDraw.text(0.2, 0.5, "black text");
        SteveDraw.setPenColor(SteveDraw.WHITE);
        SteveDraw.text(0.8, 0.8, "white text");
    }
